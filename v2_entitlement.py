...
def _trial_window(artist):
    """Return a valid trial window even for accounts created after app startup.

    New artist rows are created by auth routes while the app is already running, so
    they can legitimately have NULL trial columns until first entitlement check.
    Derive the window from created_at and persist it immediately.
    """
    if not artist:
        return None, None
    start = _dt(artist.get("trial_started_at"))
    end = _dt(artist.get("trial_ends_at"))
    if start and end:
        return start, end

    start = start or _dt(artist.get("created_at")) or datetime.now(timezone.utc)
    end = start + timedelta(days=TRIAL_DAYS)
    aid = artist.get("id")
    if aid:
        try:
            core.run(
                "UPDATE artists SET trial_started_at=?,trial_ends_at=?,subscription_status=COALESCE(NULLIF(subscription_status,''),'trialing'),updated_at=? WHERE id=?",
                (start.isoformat(), end.isoformat(), core.utcnow(), aid),
            )
            artist["trial_started_at"] = start.isoformat()
            artist["trial_ends_at"] = end.isoformat()
            if not artist.get("subscription_status"):
                artist["subscription_status"] = "trialing"
        except Exception as exc:
            print(f"Trial initialization failed // {aid}: {exc}", flush=True)
    return start, end
...
def ensure_schema():
    ...
    for name, ddl in additions:
        try:
            ...
            pass
    try:
        artists = core.all_rows("SELECT * FROM artists")
    except Exception:
        return
    for artist in artists:
        _trial_window(artist)
...
def state(artist):
    ...
    if status in ("past_due", "unpaid"):
        return "past_due"
    _start, end = _trial_window(artist)
...
async def entitlement_gate(request: Request, call_next):
    ...
    if path.startswith(_PUBLIC_PREFIXES) or path.startswith(_ALLOWED_EXPIRED_PREFIXES) or path in _ALLOWED_EXPIRED:
        return await call_next(request)
    try:
        artist = core.current_artist(request)
    except Exception:
        artist = None
    if artist and artist.get("verified") and not allowed(artist):
...
def paywall(request: Request):
    ...
    return core.page(
        ...
        f'''<div class="center"><h1>EMPTY CHAIR // {label}</h1><p>Your 7-day trial has ended.</p><p class="bright">Your chair is no longer covered.</p><div class="space"></div><p class="big">$97 / MONTH</p><a class="button" href="/billing/subscribe/monthly">KEEP MY CHAIR COVERED</a><p class="dim">Cancel anytime. Recovery resumes as soon as payment is confirmed.</p></div>''',
        chair=True,
    )


ensure_schema()
print("Empty Chair 2.0 entitlement loaded // trial initializes at account creation/first check", flush=True)
