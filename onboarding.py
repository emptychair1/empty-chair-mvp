...
def setup_shop(
    ...
    try:
        core.db_execute(
            conn,
            "UPDATE shops SET name = ?, email = ?, phone = ? WHERE id = ?",
            (name.strip(), email.strip() or None, phone.strip() or None, user["shop_id"]),
        )
        conn.commit()
    finally:
        conn.close()
...
def setup_demo(request: Request):
    ...
    try:
        artist = core.db_fetchone(
            conn,
            "SELECT id FROM artists WHERE shop_id = ? ORDER BY name LIMIT 1",
            (user["shop_id"],),
        )
        if not artist:
            ...
            core.db_execute(
                ...
                """
                INSERT INTO artists(id, shop_id, name, email, phone, styles, services, active)
                VALUES (?, ?, 'Alex Rivera', NULL, NULL, 'Traditional, Blackwork', 'tattoo', 1)
                """,
