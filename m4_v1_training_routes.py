...
def training_page(request: Request):
    ...
    return HTMLResponse(r'''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>M4 V1 Fast Train</title><style>body{margin:0;background:#080a08;color:#f2ecde;font-family:system-ui;padding:28px}.wrap{max-width:900px;margin:auto}.k{color:#c7ff3e;font-weight:800;text-transform:uppercase;font-size:12px}.card{margin-top:16px;padding:22px;border:1px solid #3b4035;background:#0d100c}.btn{border:1px solid #c7ff3e;background:#c7ff3e;color:#080a08;padding:12px 16px;font-weight:800;text-transform:uppercase;cursor:pointer}.btn:disabled{opacity:.5}.status{margin-top:16px;padding:16px;border:1px solid #30342d;background:#050705;white-space:pre-wrap;font:12px/1.5 ui-monospace,SFMono-Regular,monospace;overflow:auto}.muted{color:#989d92;line-height:1.55}</style></head><body><div class="wrap"><div class="k">M4 V1 // Fast training</div><h1>1,000,000 synthetic interactions</h1><p class="muted">750,000 train · 125,000 validation · 125,000 sealed holdout. Completed training is persisted and automatically restored here after refresh.</p><div class="card"><button id="start" class="btn">Start Fast Train</button><div id="status" class="status">Loading latest training…</div></div></div><script>(()=>{const b=document.getElementById('start'),s=document.getElementById('status');const show=x=>s.textContent=typeof x==='string'?x:JSON.stringify(x,null,2);async function req(url,opt={}){const r=await fetch(url,{...opt,cache:'no-store'});const j=await r.json();if(!r.ok)throw new Error(j.error||('HTTP '+r.status));return j;}async function poll(id){b.disabled=true;for(;;){await new Promise(r=>setTimeout(r,2000));const state=await req('/api/m4-v1/train-status?job_id='+encodeURIComponent(id));const job=state.job;if(job.status==='COMPLETED'){show({ok:true,status:'COMPLETED',result:job.result,job_id:job.id,finished_at:job.finished_at});b.disabled=false;return;}if(job.status==='FAILED'){show({ok:false,status:'FAILED',error:job.error,job_id:job.id,finished_at:job.finished_at});b.disabled=false;return;}show({ok:true,status:job.status,job_id:id,message:'Training M4 V1 against the frozen simulator…'});}}async function restore(){try{const state=await req('/api/m4-v1/train-latest');const job=state.job;if(!job){show('Ready. No prior training job found.');return;}if(job.status==='COMPLETED'){show({ok:true,status:'COMPLETED',result:job.result,job_id:job.id,finished_at:job.finished_at});return;}if(job.status==='FAILED'){show({ok:false,status:'FAILED',error:job.error,job_id:job.id,finished_at:job.finished_at});return;}show({ok:true,status:job.status,job_id:job.id,message:'Existing training job found. Resuming status polling…'});poll(job.id);}catch(e){show('ERROR loading prior training: '+e.message);}}b.onclick=async()=>{b.disabled=true;try{const start=await req('/api/m4-v1/train-job',{method:'POST'});show(start);poll(start.job_id);}catch(e){show('ERROR: '+e.message);b.disabled=false;}};restore();})();</script></body></html>''', headers={"Cache-Control": "no-store"})
...
@app.get("/api/m4-v1/train-latest")
def latest_training(request: Request):
    user, error = _user(request)
    if error:
        return error
    try:
        job = jobs.latest_job(user["shop_id"])
        return JSONResponse({"ok": True, "job": job}, headers={"Cache-Control": "no-store"})
    except Exception as exc:
        return JSONResponse({"error": str(exc)}, status_code=503, headers={"Cache-Control": "no-store"})
