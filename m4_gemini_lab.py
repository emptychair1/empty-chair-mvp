...
<script>
const go=document.getElementById('go'),state=document.getElementById('state');let ws,ctx,stream,source,processor,nextPlay=0,started=false,handshakeTimer=null;
...
async function receive(msg){let raw='';try{raw=await decodeFrame(msg.data)}catch(e){set('FAILED decoding server frame: '+e.message);return}let d;try{d=JSON.parse(raw)}catch(e){set('SERVER FRAME (not JSON): '+raw.slice(0,240));return}if(d.setupComplete){if(handshakeTimer)clearTimeout(handshakeTimer);started=true;set('M4 present · listening');ws.send(JSON.stringify({clientContent:{turns:[{role:'user',parts:[{text:'Begin now. Speak first exactly as instructed.'}]}],turnComplete:true}}));return}if(d.error){set('GEMINI ERROR: '+JSON.stringify(d.error).slice(0,360));return}if(d.serverContent){if(d.serverContent.interrupted){nextPlay=ctx.currentTime;set('interrupted · listening')}const parts=(d.serverContent.modelTurn&&d.serverContent.modelTurn.parts)||[];for(const p of parts){const blob=p.inlineData||p.inline_data;if(blob&&blob.data)playPCM(unb64(blob.data),24000)}if(d.serverContent.turnComplete)set('listening');return}set('SERVER: '+JSON.stringify(d).slice(0,360))}
async function begin(){go.disabled=true;set('requesting secure token…');try{ctx=new(window.AudioContext||window.webkitAudioContext)();await ctx.resume();const r=await fetch('/api/m4/gemini-token',{cache:'no-store'}),j=await r.json();if(!r.ok||!j.token)throw new Error(j.error||'No Gemini token');await startMic();const url='wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContentConstrained?access_token='+encodeURIComponent(j.token);ws=new WebSocket(url);ws.binaryType='arraybuffer';ws.onopen=()=>{set('connected · configuring M4');ws.send(JSON.stringify({setup:{model:'models/'+j.model,generationConfig:{responseModalities:['AUDIO'],speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:'Charon'}}}},systemInstruction:{parts:[{text:j.instructions}]}}}));handshakeTimer=setTimeout(()=>{if(!started)set('connected · no setupComplete after 8s')},8000)};ws.onmessage=receive;ws.onerror=()=>set('WebSocket error');ws.onclose=e=>{started=false;if(handshakeTimer)clearTimeout(handshakeTimer);set('connection closed '+e.code+(e.reason?' · '+e.reason:''))}}catch(e){set('FAILED: '+e.message);go.disabled=false}}
