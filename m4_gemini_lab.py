...
def gemini_token(request: Request):
    ...
    try:
        ...
        return {
            ...
            "model": GEMINI_MODEL,
            "voice": GEMINI_VOICE,
            "instructions": m4_meeting._session_instructions(user),
            "opening": m4_meeting.OPENING_INSTRUCTION,
        }
...
<body><div class="wrap"><div class="box"><div class="dot"></div><h1>M4 identity test</h1><div class="sub">actual M4 identity + actual shop state + actual constitutional values<br>Gemini Live = temporary realtime faculty · Achernar = temporary voice</div><button class="go" id="go">Meet M4 candidate</button><div id="state" class="state">Production Meeting is untouched.</div><div class="note">Pass condition: you recognize M4's mind, not merely Gemini following a role prompt.</div></div></div>
<script>
const go=document.getElementById('go'),state=document.getElementById('state');let ws,ctx,stream,source,processor,nextPlay=0,started=false,handshakeTimer=null,playSources=[];
