...
def gemini_token(request: Request):
    ...
    try:
        ...
        return {"token": token.get("name"), "model": GEMINI_MODEL, "voice": GEMINI_VOICE, "instructions": m4_meeting._session_instructions(user), "opening": LAB_OPENING}
...
<body><div class="wrap"><div class="box"><div class="dot"></div><h1>M4 identity test</h1><div class="sub">actual M4 identity + actual shop state + actual constitutional values<br>Gemini Live = temporary realtime faculty · Achernar = temporary voice</div><button class="go" id="go">Meet M4 candidate</button><div id="state" class="state">Production Meeting is untouched.</div><div class="note">Buffered audio test. M4 opens only with “Is anyone there?”</div></div></div>
<script>
const go=document.getElementById('go'),state=document.getElementById('state');let ws,ctx,stream,source,processor,started=false,handshakeTimer=null,m4Speaking=false,userSpeaking=false,lastVoiceAt=0,voiceStartedAt=0,audioQueue=[],audioQueuedSamples=0,audioSource=null,audioGeneration=0,serverTurnComplete=false;
const PLAY_RATE=24000,START_BUFFER_SAMPLES=7200,REFILL_BUFFER_SAMPLES=3600;
