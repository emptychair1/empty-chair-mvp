...
def relationship_transcript(request: Request, limit: int = 200):
    ...
    try:
        ...
        for turn in turns:
            ...
            if sid not in by_id:
                session = {"session_id": sid, "turns": []}; by_id[sid] = session; sessions.append(session)
...
function stopPlayback(){audioGeneration++;if(audioSource){try{audioSource.stop()}catch(e){}}audioSource=null;audioQueue=[];audioQueuedSamples=0;nextPlayAt=0;m4Speaking=false;serverTurnComplete=false}
function enqueuePCM(buf){const f=pcmFloat(buf);audioQueue.push(f);audioQueuedSamples+=f.length;if(!m4Speaking){m4Speaking=true;set('M4 candidate speaking')}scheduleAudio(false)}
function takeSamples(count){const out=new Float32Array(Math.min(count,audioQueuedSamples));let pos=0;while(pos<out.length&&audioQueue.length){const head=audioQueue[0],need=out.length-pos;if(head.length<=need){out.set(head,pos);pos+=head.length;audioQueue.shift()}else{out.set(head.subarray(0,need),pos);audioQueue[0]=head.subarray(need);pos+=need}}audioQueuedSamples-=out.length;return out}
function scheduleAudio(force){if(!ctx||!m4Speaking)return;const now=ctx.currentTime;if(!nextPlayAt){if(!force&&!serverTurnComplete&&audioQueuedSamples<START_BUFFER_SAMPLES)return;nextPlayAt=now+MIN_SCHEDULE_AHEAD}while(audioQueuedSamples>0&&(serverTurnComplete||audioQueuedSamples>=CHUNK_SAMPLES)){const n=serverTurnComplete?Math.min(CHUNK_SAMPLES,audioQueuedSamples):CHUNK_SAMPLES,samples=takeSamples(n),ab=ctx.createBuffer(1,samples.length,PLAY_RATE);ab.copyToChannel(samples,0);const src=ctx.createBufferSource();src.buffer=ab;src.connect(ctx.destination);const generation=audioGeneration,start=Math.max(nextPlayAt,ctx.currentTime+.015);src.start(start);nextPlayAt=start+ab.duration;audioSource=src;src.onended=()=>{if(generation!==audioGeneration)return;if(serverTurnComplete&&audioQueuedSamples===0&&ctx.currentTime>=nextPlayAt-.03){audioSource=null;nextPlayAt=0;m4Speaking=false;set('listening')}}}if(!serverTurnComplete&&audioQueuedSamples>=CHUNK_SAMPLES)setTimeout(()=>scheduleAudio(false),25)}
