...
PUBLIC_BASE_URL = os.getenv("EMPTY_CHAIR_BASE_URL", "http://localhost:8000")
...
app = FastAPI(title="Empty Chair", version="0.1.0")
...
def event(event_type, entity_type, entity_id, metadata=""):
    ...
    conn.execute(
        ...
        (event_type, entity_type, entity_id, metadata, now_iso()),
    )
    conn.commit()
    conn.close()
...
def recovery_score(customer, opening):
    ...
    if customer["completed_count"] >= 3:
        score += 10
    if customer["cancellation_count"] == 0 and customer["appointment_count"] > 0:
        score += 5

    # Recent offer fatigue penalty.
...
def send_sms(customer, opening, offer_id):
    ...
    message = (
        ...
        f"{opening['style'] or 'Tattoo'} opening with your artist is available "
        f"on {opening['date']} at {opening['start_time']} for ${opening['price']:.0f}. "
    ...
    if not all([TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER]):
        raise RuntimeError("Twilio is not configured and DEMO_MODE is false.")
...
def dashboard(request: Request):
    conn = connect()
    shop = conn.execute("SELECT * FROM shops ORDER BY created_at LIMIT 1").fetchone()
    artists = conn.execute("SELECT * FROM artists WHERE active = 1 ORDER BY name").fetchall()
...
def demo_setup():
    conn = connect()
    shop = conn.execute("SELECT id FROM shops LIMIT 1").fetchone()

    if not shop:
        shop_id = "shop_demo"
        conn.execute(
            "INSERT INTO shops(id,name,timezone,booking_url,status,created_at) VALUES (?,?,?,?,?,?)",
            (
                shop_id,
                "Demo Tattoo Studio",
                "America/New_York",
        ...
        artists = [
            ("artist_alex", "Alex", "traditional,neo_traditional", "tattoo"),
            ("artist_jordan", "Jordan", "blackwork,traditional", "tattoo"),
        ]
        for artist_id, name, styles, services in artists:
            conn.execute(
                "INSERT INTO artists(id,shop_id,name,styles,services) VALUES (?,?,?,?,?)",
                (artist_id, shop_id, name, styles, services),
            )

        customers = [
            ("cust_sarah", "Sarah Miller", "+15555550101", "artist_alex", "traditional", "tattoo", 8, 7, 0),
            ("cust_mike", "Mike Rivera", "+15555550102", "artist_alex", "traditional", "tattoo", 5, 5, 0),
            ("cust_jess", "Jessica Lee", "+15555550103", "artist_jordan", "blackwork", "tattoo", 3, 3, 0),
            ("cust_taylor", "Taylor Smith", "+15555550104", "artist_alex,artist_jordan", "traditional,blackwork", "tattoo", 2, 2, 0),
        ]
...
def create_opening(
    ...
    conn.execute(
        ...
        INSERT INTO openings(
            id,shop_id,artist_id,date,start_time,service,style,price,
    ...
    return RedirectResponse(f"/openings/{opening_id}", status_code=303)


@app.get("/openings/{opening_id}", response_class=HTMLResponse)
def opening_page(request: Request, opening_id: str):
    conn = connect()
    ...
    return templates.TemplateResponse(
        "opening.html",
        {"request": request, "opening": opening, "offers": offers},
    )
...
def start_recovery(opening_id: str):
    ...
    if opening["status"] not in ("OPEN", "RECOVERY_ACTIVE"):
        conn.close()
        raise HTTPException(400, "Opening is not available for recovery")
    ...
    for customer in customers:
        # Basic frequency protection: skip anyone offered in the last 24 hours.
        if customer["last_offer_at"]:
            try:
                last = datetime.fromisoformat(customer["last_offer_at"])
        ...
        score = recovery_score(customer, opening)
    ...
    for rank, (score, customer) in enumerate(selected, start=1):
        ...
        conn.execute(
            """
            INSERT INTO offers(
                id,opening_id,customer_id,score,rank,channel,
                expires_at,status
            )
            VALUES (?,?,?,?,?,?,?,?)
            """,
    ...
    # MVP pilot behavior: send all top candidates in demo mode.
    # In production, this should become sequential.
    for offer in offers:
        conn2 = connect()
        customer = conn2.execute(
            "SELECT * FROM customers WHERE id = ?", (offer["customer_id"],)
        ).fetchone()
        ...
        conn2.execute(
            "UPDATE customers SET last_offer_at=?, updated_at=? WHERE id=?",
            (now_iso(), now_iso(), customer["id"]),
        )
        conn2.commit()
        conn2.close()

        try:
            send_sms(customer, opening_row, offer["id"])
            event("offer.sent", "offer", offer["id"])
        except Exception as exc:
            event("offer.send_failed", "offer", offer["id"], str(exc))
...
def offer_page(request: Request, offer_id: str):
    conn = connect()
    offer = conn.execute(
        """
        SELECT o.*, c.name AS customer_name,
               op.date, op.start_time, op.end_time,
               op.service, op.style, op.price,
               a.name AS artist_name,
               s.booking_url
        FROM offers o
        JOIN customers c ON c.id = o.customer_id
        JOIN openings op ON op.id = o.opening_id
        JOIN artists a ON a.id = op.artist_id
...
def claim_offer(offer_id: str):
    ...
    offer = conn.execute(
        """
        SELECT o.*, op.status AS opening_status, op.booking_id,
               op.shop_id, op.artist_id, op.price
        FROM offers o
        JOIN openings op ON op.id = o.opening_id
        WHERE o.id = ?
        """,
    ...
    if offer["status"] in ("CLAIMED", "EXPIRED", "CANCELLED", "DECLINED"):
        conn.close()
        raise HTTPException(400, "This offer is no longer available.")

    if offer["opening_status"] not in ("OPEN", "RECOVERY_ACTIVE"):
        conn.close()
        raise HTTPException(400, "This opening is no longer available.")

    conn.execute(
        """
        UPDATE offers
        SET status='CLAIMED', claimed_at=?, responded_at=?
        WHERE id=?
        """,
        (now_iso(), now_iso(), offer_id),
    )
    ...
    # Cancel every other pending offer.
    conn.execute(
        """
        UPDATE offers
    ...
    conn.execute(
        ...
        INSERT INTO bookings(
            id,opening_id,customer_id,artist_id,
            booking_url,status,amount,booked_at
        )
        SELECT ?, o.id, ?, o.artist_id, s.booking_url,
               'PENDING', o.price, ?
        FROM openings o
...
def decline_offer(offer_id: str, reason: str = Form("skip")):
    conn = connect()
    offer = conn.execute(
    ...
    conn.execute(
        ...
        WHERE id=?
        """,
        (now_iso(), now_iso(), reason, offer_id),
    )
    conn.commit()
    conn.close()

    event("offer.declined", "offer", offer_id, reason)
    return RedirectResponse(f"/offer/{offer_id}", status_code=303)


@app.get("/booking/{booking_id}", response_class=HTMLResponse)
def booking_page(request: Request, booking_id: str):
    conn = connect()
    booking = conn.execute(
        """
        SELECT b.*, c.name AS customer_name,
               o.date, o.start_time, o.service, o.style,
               a.name AS artist_name
        FROM bookings b
...
def confirm_booking(booking_id: str):
    ...
    conn.execute(
        "UPDATE bookings SET status='CONFIRMED', booked_at=? WHERE id=?",
        (now_iso(), booking_id),
    )
    conn.execute(
