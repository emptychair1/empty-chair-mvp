...
def _ensure_boundary_table(conn):
    core.db_execute(conn, """
        CREATE TABLE IF NOT EXISTS concierge_pilot_boundaries (
...
def _pilot_boundary(conn, config):
    shop_id = str(config.get("shop_id") or "")
    display_name = str(config.get("display_name") or "").strip()
    config_updated_at = str(config.get("updated_at") or core.now_iso())
    row = core.db_fetchone(
        conn,
        "SELECT * FROM concierge_pilot_boundaries WHERE shop_id=?",
        (shop_id,),
    )
    now = core.now_iso()
    if not row:
        core.db_execute(
            conn,
            "INSERT INTO concierge_pilot_boundaries(shop_id,display_name,pilot_started_at,created_at,updated_at) VALUES (?,?,?,?,?)",
            (shop_id, display_name, config_updated_at, now, now),
        )
        conn.commit()
        return config_updated_at

    saved = dict(row)
    if str(saved.get("display_name") or "").strip() != display_name:
        core.db_execute(
            conn,
            "UPDATE concierge_pilot_boundaries SET display_name=?,pilot_started_at=?,updated_at=? WHERE shop_id=?",
            (display_name, config_updated_at, now, shop_id),
        )
        conn.commit()
        return config_updated_at

    return str(saved.get("pilot_started_at") or config_updated_at)


def _at_or_after(value, boundary):
    try:
        dt = datetime.fromisoformat(str(value or "").replace("Z", "+00:00"))
        bd = datetime.fromisoformat(str(boundary or "").replace("Z", "+00:00"))
...
def pilot_leads_live(request: Request):
    ...
    try:
        ...
        for config in configs:
            sid = str(config.get("shop_id") or "")
            boundaries[sid] = _pilot_boundary(conn, config)

        allowed = set(boundaries.keys())
        ...
        for raw in rows:
            ...
            if sid not in allowed:
                continue
            if not _at_or_after(row.get("created_at"), boundaries[sid]):
                continue
        ...
        pilots = [
            {
                "name": str(c.get("display_name") or "Tattoo Shop"),
                "leads": by_shop.get(str(c.get("shop_id") or ""), []),
            }
            for c in configs
        ]
