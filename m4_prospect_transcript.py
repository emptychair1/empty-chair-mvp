...
def session_turns(user,session_id,limit=80):
...
def latest_session(user):
    conn=core.connect()
    try:
        _ensure_table(conn);row=core.db_fetchone(conn,"SELECT session_id FROM m4_prospect_turns WHERE user_id=? AND shop_id=? ORDER BY created_at DESC LIMIT 1",(user['id'],user['shop_id']))
        if not row:return None,[]
        sid=row['session_id'];return sid,session_turns(user,sid,200)
    finally:conn.close()
...
def prospect_session(request:Request,session_id:str):
    user=core.get_current_user(request)
    if not user:return JSONResponse({'error':'Sign in first.'},status_code=401)
    turns=session_turns(user,session_id,80)
    # This is temporary meeting continuity only. It remains isolated from M4 relationship memory.
    compact=[{'speaker':t['speaker'],'text':t['text']} for t in turns[-40:]]
...
def prospect_transcript_latest(request:Request):
    user=core.get_current_user(request)
    if not user:return JSONResponse({'error':'Sign in first.'},status_code=401)
    sid,turns=latest_session(user);return JSONResponse({'session_id':sid,'count':len(turns),'turns':turns},headers={'Cache-Control':'no-store'})
...
def prospect_transcript_page(request:Request):
    ...
    sid,turns=latest_session(user);rows=[]
    for t in turns:
