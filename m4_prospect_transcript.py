...
def latest_session(user):
    conn=core.connect()
    try:
        _ensure_table(conn);row=core.db_fetchone(conn,"SELECT session_id FROM m4_prospect_sessions WHERE user_id=? AND shop_id=? ORDER BY last_activity DESC,started_at DESC LIMIT 1",(user['id'],user['shop_id']))
...
def prospect_session_status(request:Request,session_id:str):
    ...
    conn=core.connect()
    try:
        _ensure_table(conn);row=core.db_fetchone(conn,"SELECT session_id,status,turn_count,started_at,last_activity,last_error FROM m4_prospect_sessions WHERE session_id=? AND user_id=? AND shop_id=?",(session_id,user['id'],user['shop_id']))
        return JSONResponse(dict(row) if row else {'error':'Session not found.'},status_code=200 if row else 404,headers={'Cache-Control':'no-store'})
    finally:conn.close()
...
def prospect_transcript_page(request:Request):
    ...
    for t in turns:
        latency=f" · {t['latency_ms']} ms response" if t.get('speaker')=='m4' and t.get('latency_ms') is not None else '';speaker='M4' if t.get('speaker')=='m4' else 'Prospect';text=str(t.get('text') or '').replace('&','&amp;').replace('<','&lt;').replace('>','&gt;');rows.append(f"<article><div class='meta'>{speaker}{latency} · {t.get('created_at','')}</div><div class='text'>{text}</div></article>")
    body=''.join(rows) or "<p class='empty'>No turns captured yet for this recording session.</p>"
    return HTMLResponse(f"""<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>M4 Prospect Transcript</title><style>body{{margin:0;background:#f3f3ef;color:#1b1d19;font-family:system-ui}}main{{max-width:760px;margin:auto;padding:28px 18px 60px}}h1{{font:500 30px Georgia,serif}}.sid,.meta{{font:11px ui-monospace,monospace;color:#7b8076}}article{{padding:18px 0;border-top:1px solid #d8dad2}}.text{{font:17px/1.55 Georgia,serif;margin-top:7px}}</style></head><body><main><h1>Latest M4 prospect transcript</h1><div class='sid'>session: {sid or 'none'} · private · separate from relationship memory</div>{body}</main></body></html>""",headers={'Cache-Control':'no-store, no-cache, must-revalidate','Pragma':'no-cache','Expires':'0'})
