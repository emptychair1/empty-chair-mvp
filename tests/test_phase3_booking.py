...
def test_confirmed_booking_visually_fills_calendar():
    with TestClient(app) as client:
        ...
        assert 'aria-label="Appointment state calendar"' in response.text
        ...
        assert "Confirmed value" in response.text
        assert "Open slots" in response.text
        assert "In motion" in response.text
        assert 'calendar-event open' in response.text
        assert 'calendar-event working' in response.text
        assert 'calendar-event filled' in response.text
        assert 'calendar-event external' in response.text
