...
def _config_for_shop(shop_id: str, create: bool = False):
    ...
    try:
        ...
        core.db_execute(conn, "INSERT INTO concierge_pilot_config(shop_id,public_token,notification_emails,enabled,created_at,updated_at) VALUES (?,?,?,1,?,?)", (shop_id, token, "", now, now))
        conn.commit()
        row = core.db_fetchone(conn, "SELECT * FROM concierge_pilot_config WHERE shop_id=?", (shop_id,))
        return dict(row)
    finally:
...
def shop_for_public_token(token: str):
    ...
    try:
        _ensure_schema(conn)
        row = core.db_fetchone(conn, "SELECT shop_id FROM concierge_pilot_config WHERE public_token=? AND enabled=1 LIMIT 1", ((token or "").strip(),))
...
def send_new_lead_email(shop_id: str, profile: dict, confidence: int, customer_id: str = ""):
    ...
    body = f"""<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;line-height:1.55;color:#171917"><div style="font-size:12px;letter-spacing:.08em;color:#697066">EMPTY CHAIR CONCIERGE</div><h2>New lead for {e(shop_name)}</h2><table style="width:100%;border-collapse:collapse"><tr><td>Name</td><td><strong>{e(profile.get('name'))}</strong></td></tr><tr><td>Phone</td><td>{e(profile.get('phone'))}</td></tr><tr><td>Email</td><td>{e(profile.get('email'))}</td></tr><tr><td>Project</td><td>{e(profile.get('project'))}</td></tr><tr><td>Style</td><td>{e(profile.get('styles'))}</td></tr><tr><td>Placement</td><td>{e(profile.get('placement'))}</td></tr><tr><td>Budget</td><td>{e(profile.get('budget'))}</td></tr><tr><td>Timing</td><td>{e(profile.get('timing'))}</td></tr><tr><td>Artist / vibe</td><td>{e(profile.get('artist_vibe'))}</td></tr><tr><td>Preferred contact</td><td>{e(profile.get('contact_preference'))}</td></tr><tr><td>Contact permission</td><td><strong>{consent}</strong></td></tr></table></div>"""
    sent = False
...
def save_concierge_pilot_settings(request: Request, notification_emails: str = Form(""), enabled: str = Form("")):
    ...
    try:
        _ensure_schema(conn)
        core.db_execute(conn, "UPDATE concierge_pilot_config SET notification_emails=?,enabled=?,updated_at=? WHERE shop_id=?", (",".join(emails), 1 if enabled == "1" else 0, core.now_iso(), shop_id))
        conn.commit()
    finally:
        conn.close()
