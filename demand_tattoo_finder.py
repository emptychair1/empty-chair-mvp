...
def _identity_for_market(conn, market: str):
    ...
    return core.db_fetchone(
        ...
        WHERE status='active'
          AND (studio_address LIKE ? OR studio_name LIKE ?)
    ...
    try:
        ...
        if not identity:
            return core.templates.TemplateResponse(
                request=request,
                name="tattoo_finder.html",
                context={"market": market.title(), "identity": None, "error": "Tattoo Finder is not live in this market yet."},
                status_code=404,
            )
        ...
        return core.templates.TemplateResponse(
            request=request,
            name="tattoo_finder.html",
            context={"market": market.title(), "identity": identity, "campaign": campaign, "error": None},
...
def tattoo_finder_submit(
    ...
    clean = {
        ...
        "offer_opt_in": offer_opt_in == "on",
        "artist_vibe": "",
        "acquisition_mode": "tattoo_finder",
        "market": market.strip().lower(),
    }
    if not clean["project"] or not clean["placement"] or not clean["size"] or not clean["budget"] or not clean["timing"] or not clean["name"] or not clean["phone"]:
        return HTMLResponse("Please complete all required Tattoo Finder fields.", status_code=400)

    customer_id, lead_id = concierge_leads.save_concierge_profile(identity["owner_shop_id"], clean, 0)
    ...
    if campaign_id:
        demand_acquisition.record_lead_attribution(
            identity["owner_shop_id"],
            customer_id,
            campaign_id,
            source,
            identity["id"],
    ...
    return core.templates.TemplateResponse(
        request=request,
        name="tattoo_finder_complete.html",
        context={"market": market.title(), "identity": identity, "lead_id": lead_id},
        headers={"Cache-Control": "no-store"},
    )
