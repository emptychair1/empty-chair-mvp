"""Low-latency Gemini prospect Meeting transport with private transcript capture.
...
scheduled ahead. Prospect transcripts are written to the private prospect transcript
store, never to M4 relationship memory.
"""
...
let inputTranscript='',outputTranscript='',lastUserEndAt=0,firstModelAudioAt=0;
...
function mergeTranscript(current,next){next=(next||'').trim();if(!next)return current;if(!current)return next;if(next.startsWith(current))return next;if(current.endsWith(next))return current;return (current+' '+next).trim()}
async function saveTurn(speaker,text,latencyMs=null){text=(text||'').trim();if(!text)return;try{await fetch('/api/m4/prospect-turn',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({session_id:sessionId,speaker,text,latency_ms:latencyMs}),cache:'no-store'})}catch(e){console.warn('prospect transcript write failed',e)}}
function flushTurn(){const owner=inputTranscript.trim(),m4=outputTranscript.trim();const latency=(firstModelAudioAt&&lastUserEndAt)?Math.max(0,Math.round(firstModelAudioAt-lastUserEndAt)):null;inputTranscript='';outputTranscript='';firstModelAudioAt=0;if(owner)saveTurn('prospect',owner,null);if(m4)saveTurn('m4',m4,latency)}
...
function endUserTurn(){if(!userSpeaking)return;userSpeaking=false;lastUserEndAt=performance.now();send({realtimeInput:{activityEnd:{}}});set('with you')}
...
function enqueue(c){if(!firstModelAudioAt)firstModelAudioAt=performance.now();pcmQueue.push(c);queuedSamples+=Math.floor(c.byteLength/2);pump(false)}
...
function cleanup(){if(inputTranscript.trim()||outputTranscript.trim())flushTurn();started=false;userSpeaking=false;clearPlayback();try{ws&&ws.close()}catch(_){}ws=null;try{processor&&processor.disconnect()}catch(_){}processor=null;try{source&&source.disconnect()}catch(_){}source=null;try{stream&&stream.getTracks().forEach(t=>t.stop())}catch(_){}stream=null}
