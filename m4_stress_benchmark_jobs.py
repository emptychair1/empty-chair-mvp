...
def ensure_table(conn):
    core.db_execute(
        conn,
        """
        CREATE TABLE IF NOT EXISTS founder_sim_v3_benchmark_jobs (
            id TEXT PRIMARY KEY,
...
def create_job(shop_id, cycles=250):
    ...
    try:
        ...
        running = core.db_fetchone(
            conn,
            "SELECT id FROM founder_sim_v3_benchmark_jobs WHERE shop_id=? AND status='RUNNING' ORDER BY started_at DESC LIMIT 1",
            (shop_id,),
        )
        if running:
            conn.rollback()
            return str(running["id"]), False
        job_id = "founder_sim_v3_job_" + uuid.uuid4().hex
        now = core.now_iso()
        core.db_execute(
            conn,
            """INSERT INTO founder_sim_v3_benchmark_jobs(
...
def run_job(job_id, shop_id):
    ...
    try:
        ...
        row = core.db_fetchone(
            conn,
            "SELECT * FROM founder_sim_v3_benchmark_jobs WHERE id=? AND shop_id=?",
            (job_id, shop_id),
        )
    ...
    try:
        result = stress.run_benchmark(shop_id, cycles=cycles)
        done = core.connect()
        try:
            now = core.now_iso()
            core.db_execute(
                done,
                """UPDATE founder_sim_v3_benchmark_jobs
        ...
        try:
            ...
            core.db_execute(
                ...
                """UPDATE founder_sim_v3_benchmark_jobs
                   ...
                   WHERE id=? AND shop_id=?""",
                (str(exc)[:1600], now, now, job_id, shop_id),
            )
            failed.commit()
        finally:
            failed.close()
