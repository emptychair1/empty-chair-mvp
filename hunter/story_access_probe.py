...
This diagnostic tests only supported/authorized Graph API surfaces:
...
sessions/cookies, or attempt to bypass access controls.
"""
...
def probe_business_discovery(client: httpx.Client, token: str, ig_user_id: str, username: str) -> dict:
...
def classify_boundary(own: dict, external: dict | None) -> str:
    if own.get("supported"):
        return (
            "AUTHORIZED_STORIES_ONLY_UNLESS_META_DOCUMENTS_MORE"
            if external is not None
            else "AUTHORIZED_STORIES_CONFIRMED_EXTERNAL_NOT_TESTED"
        )
