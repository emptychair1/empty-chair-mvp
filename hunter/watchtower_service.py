"""Hunter Watchtower: persistent cloud browser worker and control API.

Designed to run as a standalone service, isolated from the Empty Chair customer app.
A single persistent Chromium profile is stored on disk and reused across jobs.
The API never accepts Instagram credentials and never returns browser cookies.

This service only observes pages available to its legitimately authenticated account.
It does not post, message, follow, like, solve challenges, or bypass verification.
"""
...
app = FastAPI(title="Hunter Watchtower", version="1.0.0")
...
def enqueue(username: str, collector: str) -> dict[str, Any]:
    ...
    with db() as conn:
        conn.execute(
            "INSERT INTO watchtower_jobs (id, collector, username, status, created_at) VALUES (?, ?, ?, 'queued', ?)",
            (job_id, collector, clean, created),
        )
...
def next_job() -> sqlite3.Row | None:
    with db() as conn:
        row = conn.execute(
            "SELECT * FROM watchtower_jobs WHERE status='queued' ORDER BY created_at LIMIT 1"
        ).fetchone()
        if row:
            conn.execute(
                "UPDATE watchtower_jobs SET status='running', started_at=? WHERE id=? AND status='queued'",
                (utcnow(), row["id"]),
            )
        return row
...
def finish_job(job_id: str, result: dict[str, Any]) -> None:
    with db() as conn:
        conn.execute(
            "UPDATE watchtower_jobs SET status='finished', finished_at=?, result_json=?, error=NULL WHERE id=?",
...
def fail_job(job_id: str, error: str) -> None:
    with db() as conn:
        conn.execute(
            "UPDATE watchtower_jobs SET status='failed', finished_at=?, error=? WHERE id=?",
...
def probe_story(page: Page, context: BrowserContext, username: str) -> dict[str, Any]:
    ...
    page.goto(story_url, wait_until="domcontentloaded", timeout=45000)
    page.wait_for_timeout(int(SETTLE_SECONDS * 1000))
        ...
        try:
            with sync_playwright() as p:
                ...
                while not _stop.is_set():
                    ...
                    try:
                        result = probe_story(page, context, row["username"])
                        finish_job(row["id"], result)
                        _worker_state["last_error"] = None
                    except Exception as exc:
                        message = f"{exc.__class__.__name__}: {exc}"
                        fail_job(row["id"], message)
                        _worker_state["last_error"] = message
                context.close()
        except Exception as exc:
            _worker_state["browser_started"] = False
            _worker_state["authenticated"] = False
...
def on_startup() -> None:
    global _worker
    init_db()
    _worker = threading.Thread(target=worker_loop, name="hunter-watchtower", daemon=True)
    _worker.start()
...
def healthz() -> dict[str, Any]:
    return {
        "ok": True,
        "schema": SCHEMA,
        "browser_started": _worker_state["browser_started"],
        "authenticated": _worker_state["authenticated"],
...
def status() -> dict[str, Any]:
    with db() as conn:
        counts = {
            ...
            for row in conn.execute("SELECT status, COUNT(*) AS count FROM watchtower_jobs GROUP BY status").fetchall()
        }
...
def create_job(request: JobRequest) -> dict[str, Any]:
    return enqueue(request.username, request.collector)


@app.post("/v1/jobs/batch", dependencies=[Depends(require_token)])
def create_batch(request: BatchRequest) -> dict[str, Any]:
    jobs = [enqueue(username, request.collector) for username in request.usernames]
...
def get_job(job_id: str) -> dict[str, Any]:
    with db() as conn:
        row = conn.execute("SELECT * FROM watchtower_jobs WHERE id=?", (job_id,)).fetchone()
    if not row:
...
def list_jobs(limit: int = 50) -> dict[str, Any]:
    ...
    with db() as conn:
        rows = conn.execute("SELECT * FROM watchtower_jobs ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()
    return {"count": len(rows), "jobs": [row_payload(row) for row in rows]}
