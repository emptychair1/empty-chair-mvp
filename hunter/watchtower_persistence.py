...
def lead_rankings(limit=100):
    with connection() as c:rows=c.execute(f"SELECT t.username,t.priority,t.source,COUNT(p.id) observations,COUNT(p.id) FILTER(WHERE p.signal_status='recovery_story_found') recovery_hits,COALESCE(MAX(p.intent_score),0) peak_intent,MAX(p.observed_at) last_observed_at,MAX(p.observed_at) FILTER(WHERE p.signal_status='recovery_story_found') last_recovery_at FROM {SCHEMA}.targets t LEFT JOIN {SCHEMA}.pain_ledger p ON p.username=t.username AND p.observed_at>NOW()-INTERVAL '30 days' WHERE t.enabled=TRUE GROUP BY t.username,t.priority,t.source ORDER BY (COUNT(p.id) FILTER(WHERE p.signal_status='recovery_story_found')*40+COALESCE(MAX(p.intent_score),0)+t.priority/5) DESC,MAX(p.observed_at) DESC NULLS LAST LIMIT %s",(limit,)).fetchall()
    out=[]
    for r in rows:
