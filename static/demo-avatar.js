(()=>{
  ...
  function decorate(root=document){
    ...
    root.querySelectorAll('.lead-card .lead-name').forEach(el=>{
      if(!el.closest('.identity')?.querySelector('.person-avatar,.mock-avatar')) addAvatar(el,el.textContent);
    });
  ...
  function installStyles(){
    ...
    style.textContent=`.person-with-avatar{display:inline-flex;align-items:center;gap:10px;min-width:0}.auto-person-avatar{width:42px;height:42px;border-radius:50%;border:1px solid var(--line,#3b3a32);object-fit:cover;flex:0 0 42px;background:#0b0d0b}.queue-name .auto-person-avatar{width:36px;height:36px}.rankrow .person-with-avatar,.match .person-with-avatar,.dg-name .person-with-avatar,.opening-artist .person-with-avatar,.activity-main .person-with-avatar{display:flex}.live-person .person-with-avatar{display:flex}`;
