...
def _vision_analyze(image_bytes, mime_type):
    """Return structured visual characteristics. Gemini is optional; safe fallback still ingests the image."""
    if not GEMINI_API_KEY:
        return {
            "status": "stored_pending_analysis",
            "styles": [], "motifs": [], "palette": [],
            "line_weight": "unknown", "composition": "unknown",
            "confidence": 0.0,
            "summary": "Inspiration stored. Vision analysis will run when GEMINI_API_KEY is configured."
        }
    prompt = """Analyze this tattoo inspiration image for matching, not copying. Return ONLY JSON with keys: styles (array), motifs (array), palette (array), line_weight, composition, density, negative_space, likely_scale, likely_placement (array), realism_level (0-1), color_level (0-1), confidence (0-1), summary. Do not identify a person, infer sensitive traits, or claim authorship. Describe visual tattoo characteristics only."""
    payload = {
        "contents": [{"parts": [
            {"text": prompt},
    ...
    try:
        ...
        return _json(text, {"status": "analysis_failed", "styles": [], "motifs": [], "confidence": 0})
    except Exception as exc:
        return {"status": "analysis_deferred", "styles": [], "motifs": [], "confidence": 0.0, "summary": "Image stored; analysis deferred.", "error": str(exc)[:180]}
...
def register_demand_engine(app, templates, connect, db_execute, db_fetchone, db_fetchall, login_required_redirect, now_iso=None):
    ...
    def rebuild_dna(conn, shop_id, customer_id):
        ...
        styles, motifs, palette, summaries = [], [], [], []
        confidences = []
        for asset in assets:
        ...
        for source in [customer["preferred_styles"] if customer and "preferred_styles" in customer.keys() else "", customer["tags"] if customer and "tags" in customer.keys() else ""]:
            lower = (source or "").lower()
            styles += [term for term in STYLE_TERMS if term in lower]
        def ranked(items):
            counts = {}
            for item in items:
                key = str(item).strip().lower()
                if key: counts[key] = counts.get(key, 0) + 1
            return [k for k, _ in sorted(counts.items(), key=lambda x: (-x[1], x[0]))][:12]
        dna = dict(existing)
        dna.update({"styles": ranked(styles), "motifs": ranked(motifs), "palette": ranked(palette), "visual_summary": summaries[0] if summaries else existing.get("visual_summary", ""), "inspiration_count": len(assets), "vision_confidence": round(sum(confidences)/len(confidences), 2) if confidences else 0})
    ...
    async def update_practical(request: Request, customer_id: str):
        user, redirect = login_required_redirect(request)
        if redirect: raise HTTPException(401, "Authentication required")
        payload = await request.json(); allowed = {"budget_min","budget_max","placement","timing","travel_radius_miles","short_notice","preferred_days","artist_preference"}
